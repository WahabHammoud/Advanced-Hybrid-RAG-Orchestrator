...
def create_keep_only_relevant_content_chain():
    ...
    keep_only_relevant_content_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
...
def create_question_answer_from_context_cot_chain():
    ...
    question_answer_from_context_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
...
def create_is_relevant_content_chain():
    ...
    is_relevant_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
    # is_relevant_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
...
def create_is_grounded_on_facts_chain():
    class is_grounded_on_facts(BaseModel):
        grounded_on_facts: bool = Field(description="True if answer is grounded in context, False if not")
    ...
    is_grounded_on_facts_prompt_template = """Check if this answer is grounded in the given context.
...
def create_can_be_answered_chain():
    ...
    # can_be_answered_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
    can_be_answered_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
...
def create_is_distilled_content_grounded_on_content_chain():
    ...
    # is_distilled_content_grounded_on_content_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
    is_distilled_content_grounded_on_content_llm =ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
...
def create_plan_chain():
    ...
    planner_prompt = """ For the given query {question}, create a simple step-by-step plan to find the answer. 

    IMPORTANT: Return ONLY a JSON object with a "steps" array. No explanations, no code, no additional text.

    Example format:
    {{"steps": ["Step 1: Search for information about X", "Step 2: Find details about Y", "Step 3: Answer the question based on findings"]}}
    ...
    planner_llm = ChatGroq(
        temperature=0, 
        model_name="llama3-8b-8192",  # Use smaller, more reliable model
        groq_api_key=groq_api_key, 
        max_tokens=1000
    )
...
def create_break_down_plan_chain():
    ...
    break_down_plan_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
...
def create_replanner_chain():
    ...
    replanner_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
...
def create_task_handler_chain():
    tasks_handler_prompt_template = """You are a task handler. Choose the correct tool for this task: {curr_task}

    Available tools:
    - retrieve_chunks: Search book chunks
    - retrieve_summaries: Search chapter summaries  
    - retrieve_quotes: Search book quotes
    - answer_from_context: Answer from given context

    Context available: {aggregated_context}
    Last tool used: {last_tool}
    Past steps: {past_steps}
    Original question: {question}

    IMPORTANT: Return ONLY JSON format like this:
    {{"tool": "retrieve_chunks", "explanation": "Need to search book content"}}

    JSON Response:"""

    class TaskHandlerOutput(BaseModel):
        tool: str = Field(description="Tool name: retrieve_chunks, retrieve_summaries, retrieve_quotes, or answer_from_context")
        explanation: str = Field(description="Brief explanation of why this tool was chosen")
...
def create_anonymize_question_chain():
   ...
   """
    ...
    anonymize_question_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
...
def create_deanonymize_plan_chain():
    ...
    de_anonymize_plan_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
...
def create_can_be_answered_already_chain():
    ...
    can_be_answered_already_llm = ChatGroq(temperature=0, model_name="llama3-8b-8192", groq_api_key=groq_api_key, max_tokens=2000)
